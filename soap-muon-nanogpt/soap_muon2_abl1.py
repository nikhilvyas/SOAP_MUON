...
class SOAP_Muon2_abl1(optim.Optimizer):
    """
            ...
            for p in group["params"]:
                ...
                if p.dim() == 2 and max(p.shape) < 10000:
                    ...
                    if max(p.shape) == 4*min(p.shape):
                        ...
                        norm_grad = zeropower_via_newtonschulz5(exp_avg, steps=group['newton_schulz_steps'], eps=1e-7)
                        norm_grad *= max(1, norm_grad.size(0)/norm_grad.size(1))**0.5
