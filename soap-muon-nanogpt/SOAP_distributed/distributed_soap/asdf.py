...
class DistributedSOAP(torch.optim.Optimizer):
    ...
    def _update_adam_and_momentum(
        ...
        if beta1 != 0.0:
            with DequantizeQuantizedTensorListContext(
                quantized_tensor_list = state_lists["MASKED_BLOCKED_GRADS"]
            ):
                ...
                for i, mfg in enumerate(state_lists[MASKED_FILTERED_GRAD_LIST].dequantized_value):
                    mfg.copy_(state_list_update[i])
        else:
            # Update Shampoo.
            state_lists[SHAMPOO_PRECONDITIONER_LIST].update_adam_preconditioners(
                masked_grad_list=state_lists[MASKED_BLOCKED_GRADS],
    ...
    # ) -> None:
    #     # Update Shampoo.
    #     masked_grad_list = state_lists[SHAMPOO_PRECONDITIONER_LIST].project(
    #         masked_grad_list=state_lists[MASKED_BLOCKED_GRADS],
    #         step=step,
    #     )
    ...
    def _per_group_step_impl(
        ...
        with DequantizePreconditionersContext(
            ...
            if step == 1:
                ...
                # compute_eigenvectors second arg is a boolean. turn this into an if statement. If true, rotate momentum back to orig spac eusing prev eigen vec and then compute eig and 
                # rotate to new space using the project there and project back functions
                self._compute_eigenvectors(state_lists, compute_eigenvectors)
            ...
            # V <- B_2 V + (1 - B_2) G'^2
            pre_adam = state_lists[MASKED_BLOCKED_GRADS]
            # update M' in here. Only keep M' and G.
            self._update_adam_preconditioners(
                    state_lists,
                    step,
                )
            post_adam = self._project(state_lists, step)
            print(pre_adam == post_adam)

            
            # rather than using unrotated momentum, maybe we keep rotated momentum. updated momentum with the rotated G'. However, when you run compute eigenvectors, the rotation changes
            # we must update the rotated momentum at this point in that function. use old rotatoin to project back to orig space. then update with new qr ql.
            # Maybe create our own project there and project back methods
            #  M <- B1*M + (1-B1)G
            ...
            # M' <- Q^T_L M Q_R
            # N'
            masked_blocked_search_directions = self._precondition(
                state_lists,
                masked_filtered_grad_list,
            )
