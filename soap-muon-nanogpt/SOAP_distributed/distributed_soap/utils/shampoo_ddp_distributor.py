...
from distributed_soap.utils.shampoo_block_info import DDPBlockInfo
from distributed_soap.utils.shampoo_dist_utils import get_device_mesh
from distributed_soap.utils.shampoo_distributor import DistributorInterface
...
class DDPDistributor(DistributorInterface):
    ...
    def _distribute_buffer_sizes(
        ...
        for index, aligned_buffer_size in sorted(
            ...
            if ind == 0:
                aligned_buffer_size2 = 0 # ignore first block since that is the model itself.
                ind += 1
            else:
                aligned_buffer_size2 = aligned_buffer_size
            
            heapq.heappush(
                allocated_buffer_sizes,
                (
                    min_allocated_buffer_size + aligned_buffer_size2,
                    min_allocated_buffer_size_group_index,
                ),
            )
