...
class SOAP_Muon3(optim.Optimizer):
    """
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if p.dim() == 2 and max(p.shape) < 10000:
                    ...
                    # norm_grad_adam = exp_avg_projected / denom
                    
                    norm_grad_orig = exp_avg_projected / denom**group['adam_power']
                
                    #norm_grad = state["projector"].project_back(norm_grad)
                    norm_grad_orig = self.project_back(norm_grad_orig, state)
                     
                    norm_grad = zeropower_via_newtonschulz5(norm_grad_orig, steps=group['newton_schulz_steps'], eps=1e-7)
                    norm_grad *= max(1, norm_grad.size(0)/norm_grad.size(1))**0.5
                    # norm_grad /= norm_grad.norm()
                    # norm_grad *= norm_grad_adam.norm()
