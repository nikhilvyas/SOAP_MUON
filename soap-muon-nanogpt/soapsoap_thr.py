...
class SOAPSOAP2(optim.Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "exp_avg" not in state:
                    ...
                    state["exp_avg_sq"].append(None)
                    state["exp_avg_sq"].append(torch.zeros_like(grad))
                    state["low_rank_dim"] = group["low_rank_dim"]
                ...
                if 'ortho_matrix' not in state:
                    ...
                    self.init_preconditioner( # Todo: handle state["exp_avg_sq"][1]
                        grad,
                        state,
                        precondition_frequency=group['precondition_frequency'],
                        projection_type=group['projection_type'],
                ...
                exp_avg_sq_2 = state["exp_avg_sq"][2]
                ...
                # todo: only do these if you are not only_right or only_left
                
                grad_projected2 = self.project2(grad_projected1, state)
                grad_projected2_rem = grad_projected1 - self.project2_back(grad_projected2, state)
                exp_avg_projected2 = self.project2(exp_avg_projected1, state)
                exp_avg_projected2_rem = exp_avg_projected1 - self.project2_back(exp_avg_projected2, state)
                
                exp_avg_sq_1.mul_(beta2).add_(grad_projected2.square(), alpha=(1.0 - beta2))
                exp_avg_sq_2.mul_(beta2).add_(grad_projected2_rem.square(), alpha=(1.0 - beta2))
                denom2 = exp_avg_sq_1.sqrt().add_(group["eps"])
                denom2_rem = exp_avg_sq_2.sqrt().add_(group["eps"])
                
                exp_avg_projected2 /= denom2
                exp_avg_projected2_rem /= denom2_rem
                
                norm_grad = self.project_back(exp_avg_projected2, state)
                
                # norm_grad_rem = exp_avg_projected2_rem*state["low_rank_dim"]**.5/state["full_rank_dim"]**.5
                
                norm_grad_rem -= self.project2_back(self.project2(norm_grad_rem, state), state)
                
                norm_grad_rem /= torch.norm(norm_grad_rem)
                
                norm_grad_rem *= (grad.shape[0]*grad.shape[1]*state["low_rank_dim"]/state["full_rank_dim"])**.5
                
                norm_grad += group['lr_mult']*norm_grad_rem # try .5, 1.0, 2.0
    ...
    def init_preconditioner(self, full_rank_grad, state, precondition_frequency=10, projection_type='full', shampoo_beta=0.0, max_precond_dim=10000):
        ...
        # Todo: handle state["exp_avg_sq"][1], state["full_rank_dim"], state['ortho_matrix'][??] should have one dim = state["low_rank_dim"]
