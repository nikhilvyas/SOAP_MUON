...
from iterative_qr import optimize_Q
...
class SOAP_IT(optim.Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "exp_avg" not in state:
                    ...
                    state["outer_loops"] = group["outer_loops"]
                    state["inner_loops"] = group["inner_loops"]
                    state["ns_steps"] = group["ns_steps"]
                    state["clip"] = group["clip"]
                    state["clip2"] = group["clip2"]
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        ...
        for ind, (m,o) in enumerate(zip(matrix, orth_matrix)):
            ...
            # print('o', o)
            # print(torch.max(o), torch.min(o))
            # print('m', m)
            # Q = o
            Q = optimize_Q(o, m, inner_loops=state["inner_loops"], ns_steps=state["ns_steps"], outer_loops=state["outer_loops"], clip=state["clip"], clip2=state["clip2"])
