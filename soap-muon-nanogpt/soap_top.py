    ...
    def step(self, closure = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if group["clip_threshold"] > 1e-5:
                    if max(denom.shape) == 4*min(denom.shape) or max(denom.shape) == min(denom.shape):
                        q_val = torch.quantile(denom, group["clip_threshold"])
                        denom = torch.clip(denom, min=q_val)
                        if self.print == 0:
                            print(f"Clipped denom to {q_val}")
                            self.print = 1
                            
                        denom[denom < q_val*(1.01)] /= group["clip_lr_mult"]**2
