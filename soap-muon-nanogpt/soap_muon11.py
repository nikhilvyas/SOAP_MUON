...
class SOAP_Muon11(optim.Optimizer):
    """
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if p.dim() == 2 and max(p.shape) < 10000:
                    ...
                    norm_grad2 = zeropower_via_newtonschulz5(norm_grad, steps=group['newton_schulz_steps'], eps=1e-7)
                    
                    norm_grad = torch.where(norm_grad2*norm_grad >= 0.0, torch.sqrt(norm_grad2*norm_grad), 0.0)
