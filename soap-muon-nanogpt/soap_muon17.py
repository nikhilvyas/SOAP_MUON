...
class SOAP_Muon17(optim.Optimizer):
    """
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if p.dim() == 2 and max(p.shape) < 10000:
                    ...
                    thr_val = torch.kthvalue(torch.abs(norm_grad.reshape(-1)), int(0.875*norm_grad.numel())).values
                    
                    norm_grad = torch.where(torch.abs(norm_grad) > thr_val, thr_val*torch.sign(norm_grad), norm_grad)
