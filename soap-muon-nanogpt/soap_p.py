    ...
    def step(self, closure = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                norm_grad_orig = exp_avg_projected / denom
                
                if p.dim() > 1 and max(p.shape) < 10000:
                    norm_grad = exp_avg_projected / denom**group["adam_power"]
                    norm_grad /= torch.norm(norm_grad)
                    norm_grad *= torch.norm(norm_grad_orig)
                else:
                    norm_grad = norm_grad_orig
                
                norm_grad = self.project_back(norm_grad, state, merge_dims=group["merge_dims"],
                                                 max_precond_dim=group['max_precond_dim'])
