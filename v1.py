...
class SOAP_Muon(optim.Optimizer):
    """
    ...
    def init_preconditioner(self, full_rank_grad, state, precondition_frequency=10, projection_type='full', shampoo_beta=0.0, max_precond_dim=10000):
        ...
        if projection_type == 'sp1':
            if max(full_rank_grad.shape) == 3*min(full_rank_grad.shape):
                projection_type = 'right'
            else:
                projection_type = 'full'
                
        if projection_type == 'sp2':
            if max(full_rank_grad.shape) == 3*min(full_rank_grad.shape):
                projection_type = 'right'
            else:
        ...
        if state['projection_type'] in ['std_right', 'std_right2']:
            if full_rank_grad.shape[0] > full_rank_grad.shape[1]:
                state['projection_type'] = 'right'
            elif full_rank_grad.shape[0] < full_rank_grad.shape[1]:
                state['projection_type'] = 'identity'
            else:
                if state['projection_type'] == 'std_right2':
                    state['projection_type'] = 'right'
                else:
                    state['projection_type'] = 'full'
        
        if state['projection_type'] in ['std_left', 'std_left2']:
            if full_rank_grad.shape[0] > full_rank_grad.shape[1]:
                state['projection_type'] = 'identity'
            elif full_rank_grad.shape[0] < full_rank_grad.shape[1]:
                state['projection_type'] = 'left'
            else:
                if state['projection_type'] == 'std_left2':
                    state['projection_type'] = 'left'
                else:
                    state['projection_type'] = 'full'  
